  ...
  return (
    <div className='navbar flex items-center justify-between w-full px-4 relative'>
      ...
      <div className="flex-shrink-0">
        <img src={assets.logo} alt="logo.png" className='logo w-25 h-10 mt-2' />
      </div>

      {/* Menu in the center */}
      <div className="flex-1 flex justify-center">
        <ul className={`navbar-menu gap-4 list-none text-md text-zinc-600 flex md:flex ${isMobileMenuOpen ? 'flex-col absolute top-full left-0 w-full bg-orange-200 z-10 p-4 items-center' : 'hidden md:flex'} transition-all duration-300`}>
          <li className={`cursor-pointer ${menu === 'home' ? 'border-b-2' : 'text-zinc-600 hover:bg-zinc-100'}`} onClick={() => setMenu('home')}>home</li>
