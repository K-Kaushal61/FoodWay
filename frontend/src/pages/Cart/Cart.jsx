...
const Cart = () => {
  const { cartItems, food_list, removeFromCart, getTotalCartAmount, url, isCartEmpty } = useContext(StoreContext);
    ...
    <div className='cart mt-[100px]'>
          ...
          cartItems[item._id] > 0 && (
            <div key={item._id}>
              <div className="cart-items-info grid grid-cols-[1fr_1.5fr_1fr_1fr_1fr_0.5fr] place-items-center mt-5">
                <img src={`${url}/images/${item.image}`} className='w-20 h-auto md:ml-10'/>
                <p>{item.name}</p>
                <p>₹{item.price}</p>
      ...
      <div className="cart-bottom mt-[80px] md:flex md:justify-between gap-10">
        <div className="cart-total flex-1 flex flex-col gap-10">
          ...
          {/* Desktop Button */}
          <button 
            className='border-none text-white bg-[#ff4c24] md:w-1/3 md:p-3 rounded-lg md:text-xl mb-4 hidden md:inline cursor-pointer' 
        ...
        <div className="cart-promocode flex-1 mt-5">
          ...
          <div className='flex justify-center md:hidden'>
            <button 
              className='border-none text-white bg-[#ff4c24] rounded-lg mt-4 md:hidden w-1/2 p-2 cursor-pointer'
              onClick={() => !isCartEmpty() && navigate('/placeorder')}
              disabled={isCartEmpty()}
            >
              Proceed to Checkout
